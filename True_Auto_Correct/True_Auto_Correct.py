...
class Autocompleter(plugin.Plugin):
	...
	default_words=set(['bytearray', 'elif', 'ImportError', 'RuntimeError', 'global', 'MemoryError', 'StopIteration', 'pass', 'EOFError', 'LookupError', 'BufferError', 'OSError', 'from', 'raise', 'for', 'except', 'ReferenceError', 'NameError', 'finally', 'Warning', 'import', 'ValueError', 'return', 'exec', 'else', 'assert', 'SyntaxError', 'Error', 'with', 'class', 'AssertionError', 'break', 'TypeError', 'SystemError', 'yield', 'ArithmeticError', 'while', 'continue', 'error', 'AttributeError', 'lambda'])        
	data = self.editor_s.get_text()
        current = self.text_under_cursor()
        words = set(re.split('\W+', data))+default_words
        if current in words:
